...
/**
 * Collect query stats for an insert command. Only include query shapes generated by the shell
 * that is running tests.
 *
 * @param {object} conn - connection to database
 * @param {object} options {
 ...
 * }
 */
export function getQueryStatsInsertCmd(
    conn,
    options = {
        collName: "",
        transformIdentifiers: false,
...
export function getSlowQueryLogs(testDB, queryComment, options = {}) {
    ...
    const logs = assert
        ...
        .filter((entry) => {
            ...
            // Filter by command type if specified. Matches either:
            //   - entry.attr.type === commandType (write commands that create a nested CurOp,
            //     e.g. update/delete, log a slow query entry whose 'type' is the logical op), or
            //   - entry.attr.command[commandType] is set (single-CurOp commands like insert and
            //     find log at command level with type == "command"; identify them by the command
            //     field name instead).
            if (commandType !== null) {
                const matchesType = entry.attr.type === commandType;
                const matchesCommand = entry.attr.command[commandType] !== undefined;
