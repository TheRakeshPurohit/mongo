...
TimeseriesTest.run((insert) => {
...

    {
        ...
        // Exercises argMin/argMax on the value column plus cross-column at() for the
        // time output.  Unlike $min/$max, $top/$bottom cannot be answered from the
        // bucket control document alone: the control doc records the extreme value but
        // not the row index where it occurs, and without the index we cannot look up
        // correlated fields such as the timestamp.
        //
        ...
        // argMin/argMax fast path.
        {
            const pipeline = [
                {
                    $group: {
                        ...
                        minTime: {$top: {sortBy: {value: 1}, output: "$" + timeFieldName}},
                        maxVal: {$bottom: {sortBy: {value: 1}, output: "$value"}},
                        maxTime: {$bottom: {sortBy: {value: 1}, output: "$" + timeFieldName}},
                    },
                },
                {$sort: {_id: 1}},
            ];
            ...
            assert.eq(result[0].minTime.getTime(), base.getTime() + 0, "A: min timestamp");
            assert.eq(result[0].maxVal, 9999, "A: max value");
            assert.eq(result[0].maxTime.getTime(), base.getTime() + 1499 * 1000, "A: max timestamp");
            assert.eq(result[1]._id, "B");
            assert.eq(result[1].minVal, 5, "B: min value");
            assert.eq(result[1].minTime.getTime(), base.getTime() + 1500 * 1000, "B: min timestamp");
            assert.eq(result[1].maxVal, 8888, "B: max value");
            assert.eq(result[1].maxTime.getTime(), base.getTime() + (1500 + 1099) * 1000, "B: max timestamp");
